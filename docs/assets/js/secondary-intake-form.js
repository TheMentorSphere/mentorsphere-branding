...
(() => {
  ...
  const API_ENDPOINT = '/api/forms/secondary-learner-profile';
  ...
  const isHidden = (wrapper) => {
    ...
    while (element && element !== form) {
      if (element.hidden && !element.matches('[data-step]')) return true;
  ...
  const updateYearGroupOther = () => setConditionalField('year-group-other', singleValue('learner_year_group') === 'Other / not currently following a standard school year', true);
  ...
  const updateExamBoards = () => {
    ...
    form.querySelector('[data-exam-board-help]').textContent = gcse
      ? 'For selected GCSE subjects, please add the exam board if you know it. This is optional.'
      : 'Exam boards are optional and are not usually needed for Years 7 to 9. Add a board only if it is relevant.';
  };
  const updateRelevantAreas = () => {
    const visible = singleValue('special_category_choice') === 'Yes' &&
      ...
      Boolean(namedControl('special_category_authority')?.checked) &&
  ...
  const reviewData = () => [
    ...
    },
    {
      ...
      rows: [
        ...
        ['Parental responsibility or documented legal authority', namedControl('special_category_authority')?.checked ? 'Confirmed' : 'Not applicable'],
    ...
    },
    {
      ...
      step: 4,
      rows: [
        ['Session length', multipleValues('session_length')],
        ['Session frequency', multipleValues('session_frequency')],
  ...
  const intakePayload = () => {
    ...
    return ({
    formVersion: 'secondary-learner-profile-v1',
    submissionId,
    ...
    sessionPreferences: {
      sessionLength: multipleValues('session_length'),
      sessionFrequency: multipleValues('session_frequency'),
    ...
    confirmations: {
      ...
      specialCategoryAuthority: specialCategoryProvided && Boolean(namedControl('special_category_authority')?.checked),
      learnerConsentRoute,
    },
  });
  };
  ...
  form.addEventListener('change', (event) => {
    ...
    if (
      ...
      event.target.name === 'special_category_authority' ||
