...
function classifyDuplicate_(sheet, submissionId, cache, request) {`);
 ...
 storage=storage.replace('classifyDuplicate_(sheet, request.payload.submissionId, cache);', 'classifyDuplicate_(sheet, request.payload.submissionId, cache, request);');
 storage=storage.replace("    if (duplicateStatus === 'duplicate_without_record') {", "    if (duplicateStatus === 'duplicate_conflict') {\n      return jsonOutput_({ success: false, stored: false, status: 'duplicate_conflict' });\n    }\n    if (duplicateStatus === 'duplicate_without_record') {");
 storage=storage.replace('if (!verifyStoredRow_(sheet, rowNumber, request.payload.submissionId)) {', 'if (!verifyStoredRow_(sheet, rowNumber, request.payload.submissionId) || !verifyStoredPayload_(sheet, rowNumber, request)) {');
 ...
 // Test deployments are unable to send mail, even if a recipient was accidentally configured.
 storage=storage.replace('    let notificationSent = false;\n    try {',`    let notificationSent = false;
