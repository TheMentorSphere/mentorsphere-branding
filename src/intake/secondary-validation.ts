export const FORM_VERSION = "secondary-learner-profile-v1" as const;
...
export const YEAR_GROUPS = ["Year 7", "Year 8", "Year 9", "Year 10", "Year 11", "Other / not currently following a standard school year"] as const;
...
export const WIDER_SUPPORT_OPTIONS = ["Yes, I'd like to discuss this", "No, tutoring only for now", "Maybe later", "Not sure"] as const;
...
export interface ValidatedSecondarySubmission {
    ...
    learner: {
        ...
        subjects: Subject[];
        subjectOther: string;
        examBoards: Record<Subject, string>;
    };
    supportProfile: {
...
export interface ValidatedRequest {
    submission: ValidatedSecondarySubmission;
...
function requiredText(source: Record<string, unknown>, key: string, path: string, maximum: number, errors: Record<string, string>): string {
...
function optionalText(source: Record<string, unknown>, key: string, path: string, maximum: number, errors: Record<string, string>): string {
    const value = textValue(source[key], maximum);
    if (value === null) {
...
function choice<const T extends readonly string[]>(source: Record<string, unknown>, key: string, path: string, allowed: T, errors: Record<string, string>): T[number] {
...
function optionalChoice<const T extends readonly string[]>(source: Record<string, unknown>, key: string, path: string, allowed: T, errors: Record<string, string>): T[number] | "" {
    const value = source[key];
    if (value === "")
        return "";
    if (typeof value !== "string" || !allowed.includes(value)) {
...
function choices<const T extends readonly string[]>(source: Record<string, unknown>, key: string, path: string, allowed: T, errors: Record<string, string>, required: boolean): T[number][] {
    const value = source[key];
...
export function validateSecondaryRequest(input: unknown): ValidationResult {
    ...
    const examBoards: Record<Subject, string> = { English: "", Maths: "", Science: "", Other: "" };
