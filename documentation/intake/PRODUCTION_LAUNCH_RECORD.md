...
- The response tab is named `Primary learner profiles` and has the approved 48-column consent, learner-route, authority, notification and retention schema.
...
- A narrow Gmail rule matches the exact sender, recipient and subject; applies Never Spam, the `Learner profile notifications` label and Important; and passed a fictional Inbox delivery test.
...
- The exact separate explicit-consent, learner consent-route and authority controls are implemented and independently enforced in the browser, Worker and Apps Script.
...
3. Wait for all GitHub Actions checks on the current launch commit to pass.
4. Owner explicitly approves the launch pull request. Do not merge or deploy before that approval.
5. Publish the approved Privacy Policy V1.5 before enabling the form page. If publication is not on 31 July 2026, update the effective date, policy control, change log and every corresponding publication reference to the actual date before publishing.
6. Enable `FORM_PAGE_ENABLED` only after the policy is live and verify the page in preview mode while submissions remain disabled.
7. Enable `FORM_SUBMISSIONS_ENABLED` only after a final fictional smoke test and explicit owner approval.
